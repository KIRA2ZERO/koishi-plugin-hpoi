...
function apply(ctx) {
    ctx.command('hpoi <order:string> <category:stirng> 返回hpoi维基的索引结果(默认前10条)', '手办维基索引').alias('手办维基')
        ...
        if (!(order in order_dict)) {
            session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `【${order}】不在参数列表`);
            return;
        }
        if (!(category in category_dict)) {
            session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `【${category}】不在参数列表`);
            return;
        }
        if (!(options.r18 in r18_dict)) {
            session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `【${options.r18}】不在参数列表`);
            return;
        }
        if (options.number > 36) {
            session.send((0, koishi_1.h)('quote', { id: session.messageId }) + '一次索引个数最多36个');
            return;
        }
        let r18 = options.r18, number = options.number, keyword = options.keyword;
        session.send((0, koishi_1.h)('quote', { id: session.messageId }) + '请稍等...');
